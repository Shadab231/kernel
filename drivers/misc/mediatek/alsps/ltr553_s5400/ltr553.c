...
static int ltr553_alsps_remove()
{
    struct alsps_hw *hw = get_cust_alsps_hw();
    APS_FUN();
    ltr553_power(hw, 0);
...
static void ltr553_eint_work(struct work_struct *work)
{
	...
	APS_ERR("ltr599_eint_work High: %d,Low: %d\n", hw->ps_threshold_high,hw->ps_threshold_low);
        APS_ERR("ltr599_eint_work ps_thd_val_high: %d,ps_thd_val_low: %d\n", atomic_read(&obj->ps_thd_val_high),atomic_read(&obj->ps_thd_val_low));
...
static void ps_cali_set_threshold(void)
{
    ...
    if( value_high > 0x7ef)//zhangdongfang from 0x32 to 0x7ef   this value > threshold_higd + D
    {
        value_high= 0x7ff;
        value_low= 0x7ff;
        APS_LOG("LTR553_ps_cali_set_threshold: set value_high=0x32,value_low=0x23, please check the phone \n");
    }
...
int ltr553_get_ps_status(void)
{
    ...
    // Get PS status
    ltr553_read_data_ps(obj->client, &obj->ps);
    ps_status = ltr553_get_ps_value(obj, obj->ps);	
